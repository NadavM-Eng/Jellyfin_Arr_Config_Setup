...
| MB-023 | waiting for user test | Add a command that only shows and checks an install plan. The current installer stays unchanged. | Plan checker | Test good and bad plans and make sure secrets are hidden. Then STOP: the user runs it on the Linux VM. | `added shared installation plan checker` |
