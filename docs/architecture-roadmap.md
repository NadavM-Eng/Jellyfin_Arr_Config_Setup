...
| MB-031 | waiting for user test | Add an optional Linux command that runs a plan using the existing service setup scripts. Keep `quick` unchanged. | Linux plan runner | Show the plan first. Then STOP: the user runs Quick with the new option and runs it again to check that it is safe. | `added optional Linux plan runner` plus `fixed ...` as needed. |
