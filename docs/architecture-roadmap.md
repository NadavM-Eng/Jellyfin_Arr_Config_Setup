...
| MB-040 | waiting for user test | Add a Custom command that lets the user choose parts and makes the same kind of plan as Quick. | Custom choices | Check empty, small, and full choices. Explain missing required parts. STOP: the user reviews it before Custom can install anything. | `added Custom installation choices` |
...
| MB-050 | complete | Keep one file for each Jellyfin plugin. Keep the old combined file as a fallback during migration. | Jellyfin plugins | Check every plugin file and the Quick plugin list. STOP: the user installs the plugins and runs the setup again. | Use separate `added` and `modified` commits; use `finished` only after the test. |
| MB-051 | complete | Install chosen plugins, restart Jellyfin once, wait for it, and check the result. Never remove an unchosen plugin. | Plugin runner | STOP: the user checks a safe rerun and confirms the chosen plugins are active. | `modified Jellyfin plugin setup order` |
