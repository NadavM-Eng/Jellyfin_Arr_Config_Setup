...
| MB-040 | next | Add a Custom command that lets the user choose parts and makes the same kind of plan as Quick. | Custom choices | Check empty, small, and full choices. Explain missing required parts. STOP: the user reviews it before Custom can install anything. | `added Custom installation choices` |
