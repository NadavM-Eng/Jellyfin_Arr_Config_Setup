...
| MB-030 | waiting for user test | Move small reusable jobs out of `linux-setup.sh`, one at a time. This includes messages, settings, Compose commands, folders, and checks. | Linux helper code | Check each move without running it. Then STOP after each working change so the user can rerun Quick or Verify. | One `modified ...` commit for each small move. |
