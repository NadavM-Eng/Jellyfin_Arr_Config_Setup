...
| MB-011 | next | Write down the current order: what runs first, what waits, and which parts need other parts. | Run order | Review it with the user before adding more containers, plugins, hooks, or setup scripts. | `added current installer run order` |
